...
class TranslationTagged(GenericTaggedItemBase):
    ...
    tag = models.ForeignKey(
        'dragoman_blog.UniTag', related_name="%(app_label)s_%(class)s_items")
    ...
    def tags_for(cls, model, instance=None):
        ...
        if instance is not None:
            ...
            kwargs["%s__language_code" %
                   cls.tag_relname()] = instance.language_code
        else:
...
ENTRYTRANSLATION_MODEL = getattr(
    settings, 'DRAGOMAN_BLOG_ENTRYTRANSLATION_MODEL',
    'dragoman_blog.model_defaults.EntryTranslation')
EntryTranslation = load_class(
    ENTRYTRANSLATION_MODEL, 'DRAGOMAN_BLOG_ENTRYTRANSLATION_MODEL')
